from PyQt5 import QtCore, QtWidgets  # Import the PyQt5 module we'll need
...
class MainApp(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.dbHelper = dbHelper(
            'grad.icmc.usp.br', 15215, 'orcl', 'F9875973', 'kdq6xvsm')
        ...
        self.mainwindow.dataInicial_dateEdit.setDate(
            QtCore.QDate.currentDate())
        self.mainwindow.dataFinal_dateEdit.setDate(
            QtCore.QDate.currentDate().addMonths(6))
        ...
        # Dicionário para pegar o nome do gerente a partir de um CPF
        ...
        self.mainwindow.festa_addButton.clicked.connect(
            self.on_festa_addBtn_clicked)
        self.mainwindow.festa_delButton.clicked.connect(
            self.on_festa_delBtn_clicked)
        self.mainwindow.funcionario_addButton.clicked.connect(
            self.on_funcionario_addBtn_clicked)
        self.mainwindow.funcionario_delButton.clicked.connect(
            self.on_funcionario_delBtn_clicked)
        self.mainwindow.bebida_addButton.clicked.connect(
            self.on_bebida_addBtn_clicked)
        self.mainwindow.bebida_delButton.clicked.connect(
            self.on_bebida_delBtn_clicked)
        self.mainwindow.fornecedor_addButton.clicked.connect(
            self.on_fornecedor_addBtn_clicked)
        self.mainwindow.fornecedor_delButton.clicked.connect(
            self.on_fornecedor_delBtn_clicked)
        self.mainwindow.cliente_addButton.clicked.connect(
            self.on_cliente_addBtn_clicked)
        self.mainwindow.cliente_delButton.clicked.connect(
            self.on_cliente_delBtn_clicked)
        self.mainwindow.casaDeFesta_addButton.clicked.connect(
            self.on_casaDeFesta_addBtn_clicked)
        self.mainwindow.casaDeFesta_delButton.clicked.connect(
            self.on_casaDeFesta_delBtn_clicked)
        self.mainwindow.gerente_radioButton.clicked.connect(
            self.searchFuncionarios)
        self.mainwindow.garcom_radioButton.clicked.connect(
            self.searchFuncionarios)
        self.mainwindow.operador_radioButton.clicked.connect(
            self.searchFuncionarios)
        self.mainwindow.quantidadeMin_spinBox.valueChanged.connect(
            self.spinBoxLimit)
        self.mainwindow.quantidadeMax_spinBox.valueChanged.connect(
            self.searchBebidas)
        self.mainwindow.dataInicial_dateEdit.dateChanged.connect(
            self.searchFestas)
        self.mainwindow.dataFinal_dateEdit.dateChanged.connect(
            self.searchFestas)
        self.mainwindow.casaDeFesta_comboBox.currentIndexChanged.connect(
            self.searchFestas)
        self.mainwindow.gerente_comboBox.currentIndexChanged.connect(
            self.searchFestas)
        self.mainwindow.festa_tableWidget.cellDoubleClicked.connect(
            self.editFesta)
        self.mainwindow.funcionario_tableWidget.cellDoubleClicked.connect(
            self.editFuncionario)
        self.mainwindow.bebida_tableWidget.cellDoubleClicked.connect(
            self.editBebida)
        self.mainwindow.fornecedor_tableWidget.cellDoubleClicked.connect(
            self.editFornecedor)
        self.mainwindow.cliente_tableWidget.cellDoubleClicked.connect(
            self.editCliente)
        self.mainwindow.casaDeFesta_tableWidget.cellDoubleClicked.connect(
            self.editCasaDeFesta)

        # Coloca o padrão de espaçamento dos cabeçalhos de tabelas
        # para ocupar igualmente o espaço
        for i in range(self.mainwindow.festa_tableWidget.columnCount()):
            header = self.mainwindow.festa_tableWidget.horizontalHeader()
            header.setSectionResizeMode(i, QtWidgets.QHeaderView.Stretch)
        for i in range(self.mainwindow.funcionario_tableWidget.columnCount()):
            header = self.mainwindow.funcionario_tableWidget.horizontalHeader()
            header.setSectionResizeMode(i, QtWidgets.QHeaderView.Stretch)
        for i in range(self.mainwindow.bebida_tableWidget.columnCount()):
            header = self.mainwindow.bebida_tableWidget.horizontalHeader()
            header.setSectionResizeMode(i, QtWidgets.QHeaderView.Stretch)
        for i in range(self.mainwindow.fornecedor_tableWidget.columnCount()):
            header = self.mainwindow.fornecedor_tableWidget.horizontalHeader()
            header.setSectionResizeMode(i, QtWidgets.QHeaderView.Stretch)
        for i in range(self.mainwindow.cliente_tableWidget.columnCount()):
            header = self.mainwindow.cliente_tableWidget.horizontalHeader()
            header.setSectionResizeMode(i, QtWidgets.QHeaderView.Stretch)
        for i in range(self.mainwindow.casaDeFesta_tableWidget.columnCount()):
            header = self.mainwindow.casaDeFesta_tableWidget.horizontalHeader()
            header.setSectionResizeMode(i, QtWidgets.QHeaderView.Stretch)
    ...
    # Função para trocar a quantidade de bebida
    # máxima para um a mais do que a mínima
    def spinBoxLimit(self):
        self.mainwindow.quantidadeMax_spinBox.setMinimum(
            self.mainwindow.quantidadeMin_spinBox.value()+1)
        self.searchBebidas()

    # Deleta o conteúdo da tabela de festas e preenche de
    # novo usando os parâmetros escolhidos
    def searchFestas(self):
        self.mainwindow.festa_tableWidget.clearContents()
        self.mainwindow.festa_tableWidget.setRowCount(0)

        self.fillFestas(
            self.mainwindow.dataInicial_dateEdit.date().toPyDate(),
            self.mainwindow.dataFinal_dateEdit.date().toPyDate(),
            self.mainwindow.gerente_comboBox.currentText(),
            self.mainwindow.casaDeFesta_comboBox.currentText())

    # Deleta o conteúdo da tabela de funcionários e
    # preenche de novo usando os parâmetros escolhidos
    ...
    # Deleta o conteúdo da tabela de bebidas e preenche
    # de novo usando os parâmetros escolhidos
    def searchBebidas(self):
        self.mainwindow.bebida_tableWidget.clearContents()
        self.mainwindow.bebida_tableWidget.setRowCount(0)

        self.fillBebidas(
            self.mainwindow.quantidadeMin_spinBox.value(),
            self.mainwindow.quantidadeMax_spinBox.value())
    ...
    def fillFestas(self, dataInicio, dataFinal, gerente, casaDeFesta):
        allFesta = self.dbHelper.getAllAniversarios(
            dataInicio, dataFinal, self.gerentes[gerente], casaDeFesta)
        for i, festa in enumerate(allFesta):
            ...
            for j in range(self.mainwindow.festa_tableWidget.columnCount()):
                # Converte valores nulos para um padrão
                if festa[j] == 'None':
                    festa[j] = "Não possui"
                self.mainwindow.festa_tableWidget.setItem(
                    i, j, QtWidgets.QTableWidgetItem(festa[j]))
    ...
    def fillFuncionarioGerentes(self):
        ...
        for i, funcionario in enumerate(allFuncionario):
            ...
            for j in range(
                    self.mainwindow.funcionario_tableWidget.columnCount()):
                # Converte valores nulos para um padrão
                if funcionario[j] == 'None':
                    funcionario[j] = "Não possui"
                self.mainwindow.funcionario_tableWidget.setItem(
                    i, j, QtWidgets.QTableWidgetItem(funcionario[j]))
    ...
    def fillBebidas(self, minimum, maximum):
        ...
        for i, bebida in enumerate(allBebida):
            ...
            for j in range(self.mainwindow.bebida_tableWidget.columnCount()):
                self.mainwindow.bebida_tableWidget.setItem(
                    i, j, QtWidgets.QTableWidgetItem(bebida[j]))
    ...
    def fillFornecedores(self):
        ...
        for i, fornecedor in enumerate(allFornecedor):
            ...
            for j in range(
                    self.mainwindow.fornecedor_tableWidget.columnCount()):
                # Converte valores nulos para um padrão
                if fornecedor[j] == 'None':
                    fornecedor[j] = "Não possui"
                self.mainwindow.fornecedor_tableWidget.setItem(
                    i, j, QtWidgets.QTableWidgetItem(fornecedor[j]))
    ...
    def fillClientes(self):
        ...
        for i, cliente in enumerate(allCliente):
            ...
            for j in range(self.mainwindow.cliente_tableWidget.columnCount()):
                ...
                self.mainwindow.cliente_tableWidget.setItem(
                    i, j, QtWidgets.QTableWidgetItem(cliente[j]))
    ...
    def fillCasasDeFesta(self):
        ...
        for i, casaDeFesta in enumerate(allCasaDeFesta):
            ...
            for j in range(
                    self.mainwindow.casaDeFesta_tableWidget.columnCount()):
                if casaDeFesta[j] == 'None':
                    casaDeFesta[j] = "Não possui"
                self.mainwindow.casaDeFesta_tableWidget.setItem(
                    i, j, QtWidgets.QTableWidgetItem(casaDeFesta[j]))
    ...
    def setupFesta(self):
        ...
        allFuncionario = self.dbHelper.getAllGerentes()
        for i, funcionario in enumerate(allFuncionario):
            self.festa.gerente_comboBox.addItem(
                f"{funcionario[1]} ({funcionario[0]})")

        allFuncionario = self.dbHelper.getGarconsLivres(
            self.festa.data_timeEdit.date().toPyDate())
        for i, funcionario in enumerate(allFuncionario):
            self.festa.garcons_comboBox.addItem(
                f"{funcionario[0]} ({funcionario[1]})")

        allFuncionario = self.dbHelper.getOperadoresLivres(
            self.festa.data_timeEdit.date().toPyDate())
        for i, funcionario in enumerate(allFuncionario):
            self.festa.operador_comboBox.addItem(
                f"{funcionario[0]} ({funcionario[1]})")
        ...
        # Coloca o padrão de espaçamento dos cabeçalhos
        # de tabelas para ocupar igualmente o espaço
        for i in range(self.festa.bebidas_tableWidget.columnCount()):
            header = self.festa.bebidas_tableWidget.horizontalHeader()
            header.setSectionResizeMode(i, QtWidgets.QHeaderView.Stretch)
        for i in range(self.festa.garcons_tableWidget.columnCount()):
            header = self.festa.garcons_tableWidget.horizontalHeader()
            header.setSectionResizeMode(i, QtWidgets.QHeaderView.Stretch)
        for i in range(self.festa.operador_tableWidget.columnCount()):
            header = self.festa.operador_tableWidget.horizontalHeader()
            header.setSectionResizeMode(i, QtWidgets.QHeaderView.Stretch)

        # Conecta as funções dos botões
        self.festa.buttonBox.rejected.connect(lambda: festa_addDialog.close())

        self.festa.cliente_pushButton.clicked.connect(
            self.on_festa_addToClienteBtn_clicked)
        self.festa.casaDeFesta_pushButton.clicked.connect(
            self.on_festa_addToCasaDeFestaBtn_clicked)
        self.festa.data_timeEdit.dateChanged.connect(
            self.refreshFuncionariosLivre)
        self.festa.bebida_addButton.clicked.connect(
            self.on_bebida_addToTableBtn_clicked)
        self.festa.garcons_addButton.clicked.connect(
            self.on_garcons_addToTableBtn_clicked)
        self.festa.operador_addButton.clicked.connect(
            self.on_operador_addToTableBtn_clicked)
        self.festa.bebidas_tableWidget.cellDoubleClicked.connect(
            self.rollbackBebida)
        self.festa.garcons_tableWidget.cellDoubleClicked.connect(
            self.rollbackGarcom)
        self.festa.operador_tableWidget.cellDoubleClicked.connect(
            self.rollbackOperador)
    ...
    def on_festa_addBtn_clicked(self):
        ...
        self.festa.buttonBox.accepted.connect(
            lambda: self.save_festa_and_close(festa_addDialog))
    ...
    def on_festa_delBtn_clicked(self):
        ...
        self.dbHelper.delete(
            'FESTA', ['CLIENTE', 'DATA'],
            [selectedRow[0].text(),
             datetime.strptime(selectedRow[1].text(), '%d/%m/%Y')])
    ...
    def save_festa_and_close(self, festa_addDialog):
        ...
        for i in range(self.festa.bebidas_tableWidget.rowCount()):
            bebidas.append(
                [self.festa.bebidas_tableWidget.item(i, 0).text(),
                 self.festa.bebidas_tableWidget.item(i, 1).text(),
                 self.festa.bebidas_tableWidget.item(i, 2).text()])
        ...
        for i in range(self.festa.operador_tableWidget.rowCount()):
            operadores.append(
                [i, self.festa.operador_tableWidget.item(i, 1).text()])

        # Insere nas tabelas corretas e só continua para
        # a próxima se houve êxito
        error = self.checkError(self.dbHelper.insertIntoFesta(
            [cliente, data, convidados, 'A', preco, casaDeFesta, gerente]))
        if not error:
            error = self.checkError(
                self.dbHelper.insertIntoAniversario(
                    [cliente, data, aniversariante, tema, faixa]))
        if not error:
            for bebida in bebidas:
                values = festa + bebida
                error = self.checkError(
                    self.dbHelper.insertIntoBebidaBandejaFesta(values))
        if not error:
            for garcom in garcons:
                values = festa + garcom
                error = self.checkError(
                    self.dbHelper.insertIntoGarcomFesta(values))
        if not error:
            for i, operador in enumerate(operadores):
                ...
                values += operador
                error = self.checkError(
                    self.dbHelper.insertIntoBarracaRaspadinha(values))
        if not error:   # Se não houve erros dar commit no
                        # banco de dados e atualizar a tabela de festas
    ...
    def editFesta(self):
        ...
        self.festa.buttonBox.accepted.connect(
            lambda: self.edit_festa_and_close(festa_addDialog))

        selectedRow = self.mainwindow.festa_tableWidget.selectedItems()

        # Preenche os dados do editar
        cliente = self.dbHelper.getCliente(selectedRow[0].text())[0]
        self.festa.cliente_comboBox.setCurrentIndex(
            self.festa.cliente_comboBox.findText(
                f'{cliente[1]} ({cliente[0]})'))
        self.festa.data_timeEdit.setDate(
            datetime.strptime(selectedRow[1].text(), '%d/%m/%Y'))
        ...
        self.festa.gerente_comboBox.setCurrentIndex(
            self.festa.gerente_comboBox.findText(
                f'{gerente[1]} ({gerente[0]})'))
        self.festa.casaDeFesta_comboBox.setCurrentIndex(
            self.festa.casaDeFesta_comboBox.findText(selectedRow[5].text()))
        self.festa.aniversariante_lineEdit.setText(selectedRow[6].text())
        self.festa.tema_lineEdit.setText(selectedRow[7].text())
        self.festa.faixa_comboBox.setCurrentIndex(
            self.festa.faixa_comboBox.findText(selectedRow[8].text()))

        bebidas = self.dbHelper.getBebidaBandejaFesta(
            cliente[0], datetime.strptime(selectedRow[1].text(), '%d/%m/%Y'))
        garcons = self.dbHelper.getGarconsFesta(
            cliente[0], datetime.strptime(selectedRow[1].text(), '%d/%m/%Y'))
        barracas = self.dbHelper.getBarracasAniversario(
            cliente[0], datetime.strptime(selectedRow[1].text(), '%d/%m/%Y'))
        ...
        if bebidas is not None:
            for i, bebida in enumerate(bebidas):
                ...
                self.festa.bebida_comboBox.removeItem(
                    self.festa.bebida_comboBox.findText(
                        f'{bebida[0]} ({bebida[1]}mL)'))
                for j in range(self.festa.bebidas_tableWidget.columnCount()):
                    self.festa.bebidas_tableWidget.setItem(
                        i, j, QtWidgets.QTableWidgetItem(bebida[j]))
        ...
        if garcons is not None:
            for i, garcom in enumerate(garcons):
                ...
                for j in range(self.festa.garcons_tableWidget.columnCount()):
                    self.festa.garcons_tableWidget.setItem(
                        i, j, QtWidgets.QTableWidgetItem(garcom[j]))

        if barracas is not None:
            for i, operador in enumerate(barracas):
                self.festa.operador_tableWidget.insertRow(i)
                self.festa.operador_tableWidget.setItem(
                    i, 0, QtWidgets.QTableWidgetItem(operador[1]))
                self.festa.operador_tableWidget.setItem(
                    i, 1, QtWidgets.QTableWidgetItem(operador[2]))

        # Adiciona o evento de esperar até algo ser
        # modificado para ativiar o botão de salvar
        self.festa.casaDeFesta_comboBox.currentIndexChanged.connect(
            lambda: self.festa.buttonBox.buttons()[0].setEnabled(True))
        self.festa.gerente_comboBox.currentIndexChanged.connect(
            lambda: self.festa.buttonBox.buttons()[0].setEnabled(True))
        self.festa.faixa_comboBox.currentIndexChanged.connect(
            lambda: self.festa.buttonBox.buttons()[0].setEnabled(True))
        self.festa.convidados_spinBox.valueChanged.connect(
            lambda: self.festa.buttonBox.buttons()[0].setEnabled(True))
        self.festa.preco_spinBox.valueChanged.connect(
            lambda: self.festa.buttonBox.buttons()[0].setEnabled(True))
        self.festa.casaDeFesta_comboBox.currentIndexChanged.connect(
            lambda: self.festa.buttonBox.buttons()[0].setEnabled(True))
        self.festa.gerente_comboBox.currentIndexChanged.connect(
            lambda: self.festa.buttonBox.buttons()[0].setEnabled(True))
        self.festa.aniversariante_lineEdit.textChanged.connect(
            lambda: self.festa.buttonBox.buttons()[0].setEnabled(True))
        self.festa.tema_lineEdit.textChanged.connect(
            lambda: self.festa.buttonBox.buttons()[0].setEnabled(True))
        self.festa.barracas_spinBox.valueChanged.connect(
            lambda: self.festa.buttonBox.buttons()[0].setEnabled(True))
        self.festa.faixa_comboBox.currentIndexChanged.connect(
            lambda: self.festa.buttonBox.buttons()[0].setEnabled(True))
    ...
    def edit_festa_and_close(self, festa_addDialog):
        ...
        self.dbHelper.delete(
            'BEBIDA_BANDEJA_FESTA', ['CLIENTE', 'DATA'],
            [cliente[0], datetime.strptime(selectedRow[1].text(), '%d/%m/%Y')])
        self.dbHelper.delete(
            'GARCOM_FESTA', ['CLIENTE', 'DATA'],
            [cliente[0], datetime.strptime(selectedRow[1].text(), '%d/%m/%Y')])
        self.dbHelper.delete(
            'BARRACA_RASPADINHA', ['CLIENTE', 'DATA'],
            [cliente[0], datetime.strptime(selectedRow[1].text(), '%d/%m/%Y')])
        ...
        # Atualiza as tabelas
        error = self.checkError(self.dbHelper.updateFesta(
            [cliente[0], data],
            [convidados, 'A', preco, casaDeFesta, gerente]))
        if not error:
            error = self.checkError(
                self.dbHelper.updateAniversario(
                    [cliente[0], data], [aniversariante, tema, faixa]))
        if not error:
            for bebida in bebidas:
                values = festa + bebida
                error = self.checkError(
    ...
    def setupFuncionario(self):
        ...
        self.funcionario.buttonBox.rejected.connect(
            lambda: funcionario_addDialog.close())
    ...
    def on_funcionario_addBtn_clicked(self):
        ...
        self.funcionario.buttonBox.accepted.connect(
            lambda: self.save_funcionario_and_close(funcionario_addDialog))
    ...
    def save_funcionario_and_close(self, funcionario_addDialog):
        ...
        error = self.checkError(
            self.dbHelper.insertIntoFuncionario(
                [cpf, nome, telFixo, telMovel, comissao, cargo]))

        if not error:
            if cargo == 'Gerente':  # Se o cargo for gerente adicionar
                                    # na caixa do parâmetro de pesquisa
    ...
    def editFuncionario(self):
        ...
        self.funcionario.buttonBox.accepted.connect(
            lambda: self.edit_funcionario_and_close(funcionario_addDialog))
        ...
        self.funcionario.comissao_spinBox.setValue(
            float(selectedRow[4].text()))
        if self.mainwindow.gerente_radioButton.isChecked():
        ...
        # Adiciona o evento de esperar até algo ser
        # modificado para ativiar o botão de salvar
        self.funcionario.nome_lineEdit.textChanged.connect(
            lambda: self.funcionario.buttonBox.buttons()[0].setEnabled(True))
        self.funcionario.telFixo_lineEdit.textChanged.connect(
            lambda: self.funcionario.buttonBox.buttons()[0].setEnabled(True))
        self.funcionario.telMovel_lineEdit.textChanged.connect(
            lambda: self.funcionario.buttonBox.buttons()[0].setEnabled(True))
        self.funcionario.comissao_spinBox.valueChanged.connect(
            lambda: self.funcionario.buttonBox.buttons()[0].setEnabled(True))
    ...
    def edit_funcionario_and_close(self, funcionario_addDialog):
        ...
        error = self.checkError(self.dbHelper.updateFuncionario(
            [cpf], [nome, telFixo, telMovel, comissao, cargo]))
    ...
    def setupBebida(self):
        ...
        self.bebida.buttonBox.rejected.connect(
            lambda: bebida_addDialog.close())
    ...
    def on_bebida_addBtn_clicked(self):
        ...
        self.bebida.buttonBox.accepted.connect(
            lambda: self.save_bebida_and_close(bebida_addDialog))
    ...
    def on_bebida_delBtn_clicked(self):
        ...
        self.dbHelper.delete(
            'BEBIDA', ['NOME', 'VOLUME'],
            [selectedRow[0].text(), selectedRow[1].text()])
    ...
    def save_bebida_and_close(self, bebida_addDialog):
        ...
        error = self.checkError(
            self.dbHelper.insertIntoBebida(
                [nome, volume, quantidade, bandeja, preco]))
    ...
    def editBebida(self):
        ...
        self.bebida.buttonBox.accepted.connect(
            lambda: self.edit_bebida_and_close(bebida_addDialog))
        ...
        bebida = self.dbHelper.getBebida(
            self.bebida.nome_lineEdit.text(),
            str(self.bebida.volume_spinBox.value()))[0]
        ...
        # Adiciona o evento de esperar até algo ser
        # modificado para ativiar o botão de salvar
        self.bebida.quantidade_spinBox.valueChanged.connect(
            lambda: self.bebida.buttonBox.buttons()[0].setEnabled(True))
        self.bebida.preco_spinBox.valueChanged.connect(
            lambda: self.bebida.buttonBox.buttons()[0].setEnabled(True))
        self.bebida.bandeja_checkBox.clicked.connect(
            lambda: self.bebida.buttonBox.buttons()[0].setEnabled(True))
    ...
    def edit_bebida_and_close(self, bebida_addDialog):
        ...
        error = self.checkError(self.dbHelper.updateBebida(
            [nome, volume], [quantidade, bandeja, preco]))
    ...
    def setupFornecedor(self):
        ...
        self.fornecedor.buttonBox.rejected.connect(
            lambda: fornecedor_addDialog.close())
    ...
    def on_fornecedor_addBtn_clicked(self):
        ...
        self.fornecedor.buttonBox.accepted.connect(
            lambda: self.save_fornecedor_and_close(fornecedor_addDialog))
    ...
    def save_fornecedor_and_close(self, fornecedor_addDialog):
        ...
        error = self.checkError(self.dbHelper.insertIntoDadosBancarios(
            [iddados, banco, agencia, conta, tipoConta]))
        if not error:
            error = self.checkError(self.dbHelper.insertIntoFornecedor(
                [cnpj, nome, tel, iddados]))
    ...
    def editFornecedor(self):
        ...
        self.fornecedor.buttonBox.accepted.connect(
            lambda: self.edit_fornecedor_and_close(fornecedor_addDialog))
        ...
        for b in bancos:
            if bancos[b] == banco:
                self.fornecedor.banco_comboBox.setCurrentIndex(
                    self.fornecedor.banco_comboBox.findText(b))
        ...
        self.fornecedor.nome_lineEdit.textChanged.connect(
            lambda: self.fornecedor.buttonBox.buttons()[0].setEnabled(True))
        self.fornecedor.tel_lineEdit.textChanged.connect(
            lambda: self.fornecedor.buttonBox.buttons()[0].setEnabled(True))
        self.fornecedor.banco_comboBox.currentIndexChanged.connect(
            lambda: self.fornecedor.buttonBox.buttons()[0].setEnabled(True))
        self.fornecedor.agencia_lineEdit.textChanged.connect(
            lambda: self.fornecedor.buttonBox.buttons()[0].setEnabled(True))
        self.fornecedor.conta_lineEdit.textChanged.connect(
            lambda: self.fornecedor.buttonBox.buttons()[0].setEnabled(True))
        self.fornecedor.corrente_radioButton.clicked.connect(
            lambda: self.fornecedor.buttonBox.buttons()[0].setEnabled(True))
        self.fornecedor.poupanca_radioButton.clicked.connect(
            lambda: self.fornecedor.buttonBox.buttons()[0].setEnabled(True))

        fornecedor_addDialog.exec_()

    # Armazena os dados durante a edição de um Fornecedor dado
    #  e efetua a atualização no banco de dados,caso não haja erros
    def edit_fornecedor_and_close(self, fornecedor_addDialog):
        ...
        error = self.checkError(
            self.dbHelper.updateDadosBancarios(
                [iddados], [banco, agencia, conta, tipoConta]))
        if not error:
            error = self.checkError(
                self.dbHelper.updateFornecedor([cnpj], [nome, tel, iddados]))
    ...
    def setupCliente(self):
        ...
        self.cliente.buttonBox.rejected.connect(
            lambda: cliente_addDialog.close())
    ...
    def on_cliente_addBtn_clicked(self):
        ...
        self.cliente.buttonBox.accepted.connect(
            lambda: self.save_cliente_and_close(cliente_addDialog))
    ...
    # Salva os dados da caixa de diálogo de adição de um Cliente e,
    # caso não haja erros, insere esta no banco de dados
    def save_cliente_and_close(self, cliente_addDialog):
        ...
        error = self.checkError(
            self.dbHelper.insertIntoDadosBancarios(
                [iddados, banco, agencia, conta, tipoConta]))
        if not error:
            error = self.checkError(
                self.dbHelper.insertIntoEndereco(
                    [iddados, rua, cidade, estado, numero, cep]))
        if not error:
            error = self.checkError(
                self.dbHelper.insertIntoCliente(
                    [cpf, nome, iddados, telFixo, telMovel, iddados]))
    ...
    def editCliente(self):
        ...
        self.cliente.buttonBox.accepted.connect(
            lambda: self.edit_cliente_and_close(cliente_addDialog))
        ...
        for b in bancos:
            if bancos[b] == banco:
                self.cliente.banco_comboBox.setCurrentIndex(
                    self.cliente.banco_comboBox.findText(b))
        ...
        endereco = self.dbHelper.getEnderecoCliente(
            self.cliente.cpf_lineEdit.text())[0]
        ...
        self.cliente.estado_comboBox.setCurrentIndex(
            self.cliente.estado_comboBox.findText(endereco[4]))
        self.cliente.cep_lineEdit.setText(endereco[5])

        # Conecta com os sinais que verificam alterações nos campos
        self.cliente.nome_lineEdit.textChanged.connect(
            lambda: self.cliente.buttonBox.buttons()[0].setEnabled(True))
        self.cliente.telFixo_lineEdit.textChanged.connect(
            lambda: self.cliente.buttonBox.buttons()[0].setEnabled(True))
        self.cliente.telMovel_lineEdit.textChanged.connect(
            lambda: self.cliente.buttonBox.buttons()[0].setEnabled(True))
        self.cliente.banco_comboBox.currentIndexChanged.connect(
            lambda: self.cliente.buttonBox.buttons()[0].setEnabled(True))
        self.cliente.agencia_lineEdit.textChanged.connect(
            lambda: self.cliente.buttonBox.buttons()[0].setEnabled(True))
        self.cliente.conta_lineEdit.textChanged.connect(
            lambda: self.cliente.buttonBox.buttons()[0].setEnabled(True))
        self.cliente.corrente_radioButton.clicked.connect(
            lambda: self.cliente.buttonBox.buttons()[0].setEnabled(True))
        self.cliente.poupanca_radioButton.clicked.connect(
            lambda: self.cliente.buttonBox.buttons()[0].setEnabled(True))
        self.cliente.rua_lineEdit.textChanged.connect(
            lambda: self.cliente.buttonBox.buttons()[0].setEnabled(True))
        self.cliente.n_lineEdit.textChanged.connect(
            lambda: self.cliente.buttonBox.buttons()[0].setEnabled(True))
        self.cliente.cidade_lineEdit.textChanged.connect(
            lambda: self.cliente.buttonBox.buttons()[0].setEnabled(True))
        self.cliente.estado_comboBox.currentIndexChanged.connect(
            lambda: self.cliente.buttonBox.buttons()[0].setEnabled(True))
        self.cliente.cep_lineEdit.textChanged.connect(
            lambda: self.cliente.buttonBox.buttons()[0].setEnabled(True))

        cliente_addDialog.exec_()

    # Armazena os dados durante a edição de um Cliente dado e
    # efetua a atualização no banco de dados, caso não haja erros
    def edit_cliente_and_close(self, cliente_addDialog):
        ...
        error = self.checkError(self.dbHelper.updateDadosBancarios(
            [id_dadosBancarios], [banco, agencia, conta, tipoConta]))
        if not error:
            error = self.checkError(self.dbHelper.updateEndereco(
                [id_endereco], [rua, cidade, estado, numero, cep]))
        if not error:
            error = self.checkError(self.dbHelper.updateCliente(
                [cpf],
                [nome, id_dadosBancarios, telFixo, telMovel, id_endereco]))
    ...
    def setupCasaDeFesta(self):
        ...
        # Adiciona os estados do Brasil na lista de
        # estados do endereço da Casa de Festa
        for estado in estados:
            self.casaDeFesta.estado_comboBox.addItem(estado)

        # Habilita o botão que cancela a operação na caixa de diálogo
        self.casaDeFesta.buttonBox.rejected.connect(
            lambda: casaDeFesta_addDialog.close())
    ...
    def on_casaDeFesta_addBtn_clicked(self):
        ...
        self.casaDeFesta.buttonBox.accepted.connect(
            lambda: self.save_casaDeFesta_and_close(casaDeFesta_addDialog))
    ...
    # Salva os dados da caixa de diálogo de adição de uma Casa de Festa
    # e, caso não haja erros, insere esta no banco de dados
    ...
    # Configura, preenche e exibe a caixa de diálogo
    # de edição de uma Casa de Festa
    def editCasaDeFesta(self):
        ...
        # Habilita o botão de confirmação da caixa de
        # diálogo como botão de edição
        self.casaDeFesta.buttonBox.accepted.connect(
            lambda: self.edit_casaDeFesta_and_close(casaDeFesta_addDialog))
        ...
        endereco = self.dbHelper.getEnderecoCasaFesta(
            self.casaDeFesta.nome_lineEdit.text())[0]
        self.casaDeFesta.estado_comboBox.setCurrentIndex(
            self.casaDeFesta.estado_comboBox.findText(endereco[4]))

        # Conecta com os sinais que verificam alterações nos campos
        self.casaDeFesta.rua_lineEdit.textChanged.connect(
            lambda: self.casaDeFesta.buttonBox.buttons()[0].setEnabled(True))
        self.casaDeFesta.n_lineEdit.textChanged.connect(
            lambda: self.casaDeFesta.buttonBox.buttons()[0].setEnabled(True))
        self.casaDeFesta.cidade_lineEdit.textChanged.connect(
            lambda: self.casaDeFesta.buttonBox.buttons()[0].setEnabled(True))
        self.casaDeFesta.estado_comboBox.currentIndexChanged.connect(
            lambda: self.casaDeFesta.buttonBox.buttons()[0].setEnabled(True))
        self.casaDeFesta.cep_lineEdit.textChanged.connect(
            lambda: self.casaDeFesta.buttonBox.buttons()[0].setEnabled(True))

        casaDeFesta_addDialog.exec_()

    # Armazena os dados durante a edição de uma Casa de Festa dada e efetua
    # a atualização no banco de dados, caso não haja erros
    ...
    # Remove uma Bebidade de uma Festa e retorna ela para
    # a lista de disponíveis para a festa
    def rollbackBebida(self):
        self.festa.buttonBox.buttons()[0].setEnabled(True)

        selectedRow = self.festa.bebidas_tableWidget.selectedItems()
        self.festa.bebida_comboBox.addItem(
            f"{selectedRow[0].text()} ({selectedRow[1].text()}mL)")
        self.festa.bebidas_tableWidget.removeRow(
            self.festa.bebidas_tableWidget.row(selectedRow[0]))

    # Remove um Garcom de uma Festa e retorna ele
    # para a lista de disponíveis no dia
    def rollbackGarcom(self):
        ...
        self.festa.garcons_comboBox.addItem(
            f"{selectedRow[0].text()} ({selectedRow[1].text()}mL)")
        self.festa.garcons_tableWidget.removeRow(
            self.festa.garcons_tableWidget.row(selectedRow[0]))

    # Remove um Operador de Raspadinha de uma Festa e retorna
    # ele para a lista de disponíveis no dia
    def rollbackOperador(self):
        ...
        self.festa.operador_comboBox.addItem(
            f"{selectedRow[0].text()} ({selectedRow[1].text()}mL)")
        self.festa.operador_tableWidget.removeRow(
            self.festa.operador_tableWidget.row(selectedRow[0]))

    # Atualiza a lista de funcionários disponíveis para
    # uma Festa na tela de adição/edição desta
    ...
    # Ativa a caixa de diálogo para adição de um
    # Cliente na tela de adição/edição de Festa
    ...
    # Ativa a caixa de diálogo para adição de uma Casa de Festa
    # na tela de adição/edição de Festa
    ...
    # Adiciona a Bebida escolhida e a quantidade da mesma a
    # uma Festa, durante adição/edição desta
    def on_bebida_addToTableBtn_clicked(self):
        ...
        if nome != '':
            self.festa.bebida_comboBox.removeItem(
                self.festa.bebida_comboBox.currentIndex())
            ...
            self.festa.bebidas_tableWidget.setItem(
                numRows, 0, QtWidgets.QTableWidgetItem(match[1][:-1]))
            self.festa.bebidas_tableWidget.setItem(
                numRows, 1, QtWidgets.QTableWidgetItem(match[2]))
            self.festa.bebidas_tableWidget.setItem(
                numRows, 2, QtWidgets.QTableWidgetItem(quantidade))
    ...
    def on_garcons_addToTableBtn_clicked(self):
        ...
        if nome != '':
            self.festa.garcons_comboBox.removeItem(
                self.festa.garcons_comboBox.currentIndex())
    ...
    # Adiciona o Operador de Raspadinha escolhido a uma Festa,
    # durante a adição/edição desta
    def on_operador_addToTableBtn_clicked(self):
        self.festa.buttonBox.buttons()[0].setEnabled(True)
        nome = self.festa.operador_comboBox.currentText()

        if nome != '' and \
                self.festa.barracas_spinBox.value() > \
                self.festa.operador_tableWidget.rowCount():
            self.festa.operador_comboBox.removeItem(
                self.festa.operador_comboBox.currentIndex())
